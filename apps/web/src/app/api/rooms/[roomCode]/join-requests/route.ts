...
// GET: list pending join requests (host only)
export async function GET(_req: Request, { params }: { params: { roomCode: string } }) {
  ...
  const res = await fetch(`${API_INTERNAL_URL}/v1/rooms/${params.roomCode}/join-requests`, {
    headers: { Authorization: `Bearer ${INTERNAL_API_SECRET}`, 'x-user-id': session.user.id },
  });
  return NextResponse.json(await res.json(), { status: res.status });
}

// POST: submit a join request (public - any peer can submit)
