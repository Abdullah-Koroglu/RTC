import { NextResponse } from 'next/server';
import { getServerSession } from 'next-auth';
import { authOptions } from '@/lib/auth';
import { getServerEnv } from '@/lib/env';

function internalHeaders(secret: string, userId: string) {
  return { Authorization: `Bearer ${secret}`, 'x-user-id': userId };
}

// GET /api/contacts/requests?direction=incoming|outgoing
export async function GET(req: Request) {
  const session = await getServerSession(authOptions);
  if (!session?.user?.id) return NextResponse.json({ error: 'Unauthorized' }, { status: 401 });

  const { searchParams } = new URL(req.url);
  const direction = searchParams.get('direction') === 'outgoing' ? 'outgoing' : 'incoming';

  const { API_INTERNAL_URL, INTERNAL_API_SECRET } = getServerEnv();
  const res = await fetch(`${API_INTERNAL_URL}/v1/contacts/requests/${direction}`, {
    headers: internalHeaders(INTERNAL_API_SECRET, session.user.id),
  });
  const data = await res.json() as unknown;
  return NextResponse.json(data, { status: res.status });
}
